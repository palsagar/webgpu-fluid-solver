...
4. **Blow mode** — the default mouse mode (CONTEXT.md vocabulary): dragging injects momentum + Smoke at the cursor, a moving momentum source, not a special effect; the current default (dragging the Obstacle) becomes a non-default mode, switched by explicit toggle, never implicit gesture. Build notes: (a) all writes must hit **all three rotation slots** — `writeVelocityU`/`writeVelocityV`/`writeSmoke` already fan out to every slot, reuse them; (b) no solver change is needed — projection runs every step, so injected momentum is made divergence-free on the next dispatch; (c) do **not** copy the obstacle drag path's upload pattern (whole-field CPU arrays on every `mousemove`, 3 × 4 MB per component at tier 1024 — see the upload-cost gap below): a Blow splat is local, so write a bounded region around the cursor, throttled, rather than mirroring whole fields on the CPU.

5. **Freehand Draw mode + ε slider** — rasterize drawn solids into the Solid Mask (needs a new incremental invalidation path and an eraser counterpart); Confinement exposed as labeled-artificial, default-off, per ADR-0006's standing policy. Build notes: (a) **hard prerequisite — the field-reset defect below.** Draw rasterizes solids repeatedly during a stroke; on today's code every rasterization ends by pushing the stale CPU mirrors `_uData`/`_vData` over the whole live field, i.e. every Draw stroke would restart the flow. Fix that first (GPU-side rasterizer, per the sequencing below) and Draw mode inherits the fix; (b) eraser semantics are an open design question — a newly-fluid cell has no meaningful velocity; zeroing it and letting projection/advection refill is the simple option, interpolating from fluid neighbours the smoother one — decide and record it in the PR; (c) the ε slider must read as artificial in the UI and default to off — no silent vorticity injection, ever (ADR-0006).

### Sequencing for the next PRs

The gaps below are not independent of the features — three of them sit on the code path Draw mode must build on. Suggested order, each its own PR:

- **PR A — GPU-side obstacle rasterization (the field-reset fix).** Move `rasterizeObstacle()`'s work to the GPU (or refresh the CPU mirrors by readback): write the solid mask and the obstacle velocity without round-tripping stale whole-field arrays. Closes the **field-reset defect** (user-visible on the flagship demo today) and subsumes the **drag-time upload-cost gap** (same code path), and it is the platform Draw mode rasterizes on. While touching buffer recreation: the **rotation-invariant-after-resize/tier-change test gap** is cheap to close in the same PR.
- **PR B — moving-wall viscous BC.** Today `diffuse.wgsl` ghosts a buried face to `-center`, pinning a *dragged* obstacle's wall at zero. Ghost against the stored wall velocity instead. Only meaningful after PR A — the field reset currently masks it — and the viscous stencil change wants a measurement of the near-wall shear before/after, per this branch's every-number-is-measured rule.
...
- **Several assertions pin the live solver against offline-transcribed reference values** the suite never recomputes — the onset `52.2`, the operating-point `ν_num = 7.7808e-4`, the St band. The tolerances are justified in-file, but the reference values come from multi-minute offline sweeps not in the repo, so a failure cannot be told from a stale transcription without re-running them.
