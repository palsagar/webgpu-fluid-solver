...
3. **Viscosity + Re slider + Strouhal readout** — explicit diffusion pass; Re capped to the measured honest window (slider 0.25–500; at the startup tier, 4.1 – 154 at the shipped 256 pressure iterations); live St from a downstream Probe on the Kármán preset — still open. Flagship demo: vortex street dies below the **measured** onset Re ≈ 52.2 ± 0.3, found by fitting the perturbation's exponential growth rate and locating where it crosses zero — above the textbook unconfined 47, in the direction the channel blockage (`D/H = 0.12`) predicts. [ADR-0007]
