...
`_endDrag()` previously set `dragging = false` and nothing else: the last mousemove's velocity stayed in the solid cells until the next rasterize. With advection alone that was bounded and shipped; with the ghost reading it, a drag ending on a fast flick would leave the viscous pass pumping momentum toward a phantom moving wall on every subsequent frame — up to 32 substeps each — indefinitely. `_endDrag()` now re-rasterizes at the current position with zero velocity: one dispatch round, the same cost as a single mousemove, matching the zero-velocity pattern `_startDrag` and `_rotate` already use. The release listener is bound on `window`, so a drag released off-canvas still ends (and zeroes) the drag; `touchcancel` and window blur also route through `_endDrag()`, so canceled input cannot strand wall velocity. Shared preset loading ends an active drag before resetting/applying preset state. Obstacle-less presets reject obstacle drag and Shift-rotation paths, preventing hidden solids. Mid-drag *holds* (mouse down, not moving) keep the last drag velocity — that is shipped advection behavior pinned by PR A's tests, and redefining it is out of scope.

> **Amendment (5af3911, insert-on-click):** the mousedown path of this guard is relaxed — the first canvas pointerdown in an obstacle-less preset (backward-step) inserts the obstacle and sets `interaction.showObstacle = true` (interaction.js:164), so the overlay ring, shape buttons, and Re/St badges acknowledge the now-visible body. The Shift+mousemove phantom-rotation guard (interaction.js:198) is unchanged.
...
1. **Direction and magnitude (measured).** After K steps the near-wall fluid velocity ends strictly closer to `vx` than the identical replay on master. The closure factor is recorded here as a measured number. Measured on the scripted-drag harness (quieted field, `vx = 1.0`, 4 substeps at coeff 0.2, K = 40 steps, boot tier): near-wall mean `u` = **-0.0055895052864798345** on master @ `9e05cf7` against **0.576810504309833** on this branch — the ring closes from -0.56% to 57.7% of `vx`, a 2.38× reduction of the deficit. Per-step near-wall extrema stayed within [0.3887721598148346, 0.8068600296974182], inside the measured gate band. The pre-fix branch run reproduced the master value bit-exactly, so the delta is attributable to the stencil change alone.
2. **No overshoot (measured bound).** With one mask-buried neighbour the ghost is `2w − c`, so the fluid-face update is `(1 − 5·coeff)·c + coeff·(2w + n2 + n3 + n4)`. The center weight is negative for `coeff > 1/5`, so the update is a convex combination only for `coeff ≤ 1/5`; at saturation (`coeff = 1/4`) it is not convex and a bounded transient overshoot beyond `vx` is possible, up to `(5·coeff − 1)·local range` per substep (≤ 25% of the local range at `coeff = 1/4`). This non-convexity is pre-existing — the old `-center` ghost has the same center weight — not introduced by ADR-0011. The gate runs at `coeff = 0.2` (convex), and the measured per-step near-wall extrema stayed within `[0.3887721598148346, 0.8068600296974182]` — asserted in-suite per step, not sampled at the end.
