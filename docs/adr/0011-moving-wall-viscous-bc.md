...
1. **Direction and magnitude (measured).** After K steps the near-wall fluid velocity ends strictly closer to `vx` than the identical replay on master. The closure factor is recorded here as a measured number. *(before/after numbers appended at implementation time.)*
