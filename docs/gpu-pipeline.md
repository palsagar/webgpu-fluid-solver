...
**diffuse.wgsl:** Classifies each velocity *face* three ways — FLUID (both flanking cells fluid, diffused), WALL (exactly one solid, copied through, preserving the no-penetration BC and the inflow column), BURIED — split in two: **mask-buried** faces (both flanking cells solid per the Solid Mask) ghost to `w + (w - center)` with `w` the face's **own stored value** (the rasterizer-written wall velocity), and **index-buried** faces (`i == 0` / `j == 0`) plus **u-faces on the domain top row** (`j == numY - 1`, whose stored `u` is `boundary.wgsl`'s zero-gradient free-stream extrapolation, not a wall velocity) ghost to `-center`. The index-based part of the BURIED test is load-bearing: at `j = 0` for `u` and `i = 0` for `v` both flanking cells are fluid, so the FLUID predicate does not block the read, and the mask test itself would underflow. The domain ring (first and last rows and columns) is copied through unchanged, which is why those boundary lines carry no viscous update.
